...
public class PriceView {
  ...
  public static PriceView fromEntity(Price price) {
    return PriceView.builder()
        .symbol(price.getSymbol().getName())
        .premium(price.getPremium())
        .upbit(price.getUpbit())
        .binance(price.getBinance())
        .upbitTradeAt(price.getUpbitTradeAt())
        .binanceTradeAt(price.getBinanceTradeAt())
