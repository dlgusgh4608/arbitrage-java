...
public class BuyOrderService {
    ...
    public BuyOrderResDto createMarketBuyOrder(
        User user,
        Symbol symbol,
        ExchangeRate exchangeRate,
        BinanceOrderResponseDto binanceOrderRes,
        UpbitGetOrderResponseDto upbitOrderRes
    ) {
        ...
        double binanceCommission = MathUtil.roundTo(binanceTotalPrice * BINANCE_TAKER_COMM, 8); // 바이낸스 수수료
        ...
        buyOrderRepository.save(
            BuyOrder.builder()
                ...
                .upbitQuantity((float) upbitQty)
                .upbitCommission((float) upbitCommission)
                .binancePrice((float) binanceAvgPrice)
                .binanceQuantity((float) binanceQty)
                .binanceCommission((float) binanceCommission)
                .isMaker(false)
                .isClose(false)
                .build()
        );

        return BuyOrderResDto.builder()
            .premium(premium)
            .binanceAvgPrice(binanceAvgPrice)
            .binanceQty(binanceQty)
            .binanceTotalPrice(binanceTotalPrice)
            .binanceCommission(binanceCommission)
            .upbitTotalPrice(upbitTotalPrice)
            .upbitQty(upbitQty)
            .upbitAvgPrice(upbitAvgPrice)
