...
public class SellOrderService {
        ...
        public SellOrder createMarketOrder(OrderCalcResult orderCalcResult,
                        ...
                        UpbitGetOrderResponse upbitGetOrderResponse, ExchangeRate exchangeRate) {
                ...
                double binanceCommission =
                                MathUtil.roundTo(binanceTotalPrice * BINANCE_TAKER_COMM_RATE, 8);
                ...
                double premium = MathUtil.calculatePremium(upbitAvgPrice, binanceAvgPrice,
                                usdToKrw);
                double exchangeRateAtBuy =
                                orderCalcResult.getBuyOrder().getExchangeRate().getRate();
                double premiumInBuyOrder = orderCalcResult.getBuyOrder().getPremium();
                double premiumWithBuyExchangeRate = MathUtil.calculatePremium(upbitAvgPrice,
                                binanceAvgPrice, exchangeRateAtBuy);

                double profitRate =
                                MathUtil.roundTo(premiumInBuyOrder - premiumWithBuyExchangeRate, 4);


                SellOrder sellOrder = SellOrder.builder().buyOrder(orderCalcResult.getBuyOrder())
                                .exchangeRate(exchangeRate).premium(premium)
                                .upbitPrice(upbitAvgPrice).upbitQuantity(upbitQty)
                                .upbitCommission(upbitCommission).binancePrice(binanceAvgPrice)
                                .binanceQuantity(binanceQty).binanceCommission(binanceCommission)
                                .isMaker(false).profitRate(profitRate).build();
