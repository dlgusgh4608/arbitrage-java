...
public class OrderApplicationService {
    ...
    public BuyOrderResDto createBuyOrder(BuyOrderReqDto req) throws Exception {
        ...
        BinanceOrderResponseDto binanceOrderRes = binanceService.order( // 시장가 숏
            symbol.getName(),
            BinanceOrderEnum.Side.SELL,
            BinanceOrderEnum.Type.MARKET,
            req.getQty(),
            null
        );

        String uuid = upbitService.order(
            symbol.getName(),
            UpbitOrderEnum.Side.bid,
        ...
        return buyOrderService.createMarketBuyOrder(user, symbol, fixedExchangeRate, binanceOrderRes, upbitOrderRes);
    }
}